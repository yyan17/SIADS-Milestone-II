...
def crawling_stat(team_url):
    ...
    WebDriverWait(team_driver, 10).until(is_ready)
    ...
    for i, element in enumerate(summary_elements):
        ...
        team_dict['G-PK'] = get_text_strip(td_elements[9])
        team_dict['PK'] = get_text_strip(td_elements[10])
        team_dict['PKatt'] = get_text_strip(td_elements[11])
        team_dict['CrdY'] = get_text_strip(td_elements[12])
        team_dict['CrdR'] = get_text_strip(td_elements[13])
        team_dict['xG'] = get_text_strip(td_elements[14])
        team_dict['npxG'] = get_text_strip(td_elements[15])
        team_dict['xAG'] = get_text_strip(td_elements[16])
        team_dict['npxG+xAG'] = get_text_strip(td_elements[17])
        team_dict['PrgC'] = get_text_strip(td_elements[18])
        team_dict['PrgP'] = get_text_strip(td_elements[19])
    ...
    opponent_info.find_element(By.CSS_SELECTOR, "a").click()
    time.sleep(API_DELAY_TERM)
    ...
    for i, element in enumerate(opponent_elements):
        ...
        team_dict['G-PK_Conceded'] = get_text_strip(td_elements[9])
        team_dict['PK_Conceded'] = get_text_strip(td_elements[10])
        team_dict['PKatt_Conceded'] = get_text_strip(td_elements[11])
        team_dict['CrdY_Conceded'] = get_text_strip(td_elements[12])
        team_dict['CrdR_Conceded'] = get_text_strip(td_elements[13])
        team_dict['xG_Conceded'] = get_text_strip(td_elements[14])
        team_dict['npxG_Conceded'] = get_text_strip(td_elements[15])
        team_dict['xAG_Conceded'] = get_text_strip(td_elements[16])
        team_dict['npxG+xAG_Conceded'] = get_text_strip(td_elements[17])
        team_dict['PrgC_Conceded'] = get_text_strip(td_elements[18])
        team_dict['PrgP_Conceded'] = get_text_strip(td_elements[19])
...
def save_team(league):
    ...
    for year in years:
        ...
        data.to_csv(f"../datasets/teams/fbref.com/team_stats_{year}.csv")
